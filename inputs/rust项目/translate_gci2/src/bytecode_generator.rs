...
pub fn bytecode_generator_conf(bc_gen: &mut BYTECODE_GENERATOR, ast: &'static UNIT_AST) {
    bc_gen.ast = Some(ast);
...
fn property_ast_bytecode_generate(bc_gen: &mut BYTECODE_GENERATOR, ast: &PROPERTY_AST) -> i32 {
...
fn number_ast_bytecode_generate(bc_gen: &mut BYTECODE_GENERATOR, ast: &NUMBER_AST) -> i32 {
...
fn variable_ast_bytecode_generate(bc_gen: &mut BYTECODE_GENERATOR, ast: &VARIABLE_AST) -> i32 {
...
fn len_expr_ast_bytecode_generate(bc_gen: &mut BYTECODE_GENERATOR, ast: &LEN_EXPR_AST) -> i32 {
...
fn primary_expr_ast_bytecode_generate(
    ...
    match ast {
        ...
        PRIMARY_EXPR_AST::Len(len_expr) => len_expr_ast_bytecode_generate(bc_gen, len_expr),
        PRIMARY_EXPR_AST::FunctionCall(fn_call) => {
            function_call_expr_ast_bytecode_generate(bc_gen, fn_call)
        }
        PRIMARY_EXPR_AST::Variable(var) => variable_ast_bytecode_generate(bc_gen, var),
        PRIMARY_EXPR_AST::Number(num) => number_ast_bytecode_generate(bc_gen, num),
...
fn eq_expr_ast_bytecode_generate(bc_gen: &mut BYTECODE_GENERATOR, ast: &EQ_EXPR_AST) -> i32 {
...
fn decl_stmt_ast_bytecode_generate(bc_gen: &mut BYTECODE_GENERATOR, ast: &DECL_STMT_AST) -> i32 {
...
fn while_stmt_ast_bytecode_generate(bc_gen: &mut BYTECODE_GENERATOR, ast: &WHILE_STMT_AST) -> i32 {
...
fn stmt_ast_bytecode_generate(
    ...
    match ast {
        ...
        STMT_AST::FunctionCall(fn_call) => {
            function_call_stmt_ast_bytecode_generate(bc_gen, fn_call)
        }
        STMT_AST::If(if_stmt) => {
            if_stmt_ast_bytecode_generate(bc_gen, if_stmt, loop_start_idx, loop_exit_idxs)
        }
        STMT_AST::While(while_stmt) => while_stmt_ast_bytecode_generate(bc_gen, while_stmt),
        STMT_AST::Break(break_stmt) => {
            break_stmt_ast_bytecode_generate(bc_gen, break_stmt, loop_exit_idxs)
        }
        STMT_AST::Continue(continue_stmt) => {
            continue_stmt_ast_bytecode_generate(bc_gen, continue_stmt, loop_start_idx)
        }
...
pub fn bytecode_generator_generate(
    ...
    if let Some(ref ast) = bc_gen.ast {
        if !ast.functions.is_empty() {
            ...
            let r = body_ast_bytecode_generate(
                bc_gen,
                &f.body,
                &mut loop_start_idx,
                &mut loop_exit_idxs,
    ...
    for (i, cnst) in bc.constant_pool.iter().enumerate() {
        match &cnst.type_ {
            ...
            CONSTANT_TYPE::CONSTANT_TYPE_FUNCTIONREF => {
                ...
                let _ = writeln!(
                    f,
                    "\t\t<functionref_cnst idx=\"{}\">{}</functionref_cnst>",
                    i, s
                );
            }
        }
