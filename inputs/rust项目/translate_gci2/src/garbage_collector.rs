...
unsafe fn lookup_new_location(gc: garbage_collector_type_t, ptr_val: *mut u8) -> *mut u8 {
    ...
    if ptrs_map_get(&(*gc).ptrs_map, o as *mut std::ffi::c_void).is_null() {
        ptrs_map_set(
            &mut (*gc).ptrs_map,
            o as *mut std::ffi::c_void,
            (*(*gc).b.free_list.first) as *mut std::ffi::c_void,
        );
    ...
    ((ptrs_map_get(&(*gc).ptrs_map, o as *mut std::ffi::c_void) as *mut u8).offset(1)) as *mut u8
}
...
unsafe fn run_gc(gc: garbage_collector_type_t, ptr_ref: *mut *mut u8, ptr_sizemem: usize) {
...
unsafe fn change_one_ptr(gc: garbage_collector_type_t, prev_ptr: *mut u8, new_ptr: *mut u8) {
    let stack = *(*gc).stack;
    let stack_top = *(*gc).stack_top;
...
pub fn garbage_collector_malloc_arr(gc: garbage_collector_type_t, arr_len: usize) -> *mut ARRAY {
    unsafe {
        let start_arr_cap = arr_len * 2;
        let sizemem = mem::size_of::<u8>()
            + mem::size_of::<ARRAY>()
            + mem::size_of::<VALUE>() * start_arr_cap;
...
pub fn garbage_collector_realloc_arr(
    ...
    unsafe {
        ...
        let sizemem =
            mem::size_of::<u8>() + mem::size_of::<ARRAY>() + mem::size_of::<VALUE>() * new_arr_cap;
...
// These helpers mirror the block layout logic from `src/allocator.rs`.
// They are duplicated here to avoid depending on a C build for GC internals.
...
unsafe fn BLOCK_L_DATA_LEN(block: *mut u8) -> usize {
    *((block as usize + std::mem::size_of::<char>()) as *const usize)
}

unsafe fn BLOCK_LIST_NEXT(block: *mut u8) -> *mut u8 {
    *((block as usize
        + std::mem::size_of::<char>()
        + std::mem::size_of::<usize>()
        + std::mem::size_of::<*mut u8>()) as *const *mut u8)
}

unsafe fn BLOCK_DATA(block: *mut u8) -> *mut u8 {
    (block as usize
        + std::mem::size_of::<char>()
        + std::mem::size_of::<usize>()
...
unsafe fn BLOCK_PTR_FROM_DATA(ptr: *mut u8) -> *mut u8 {
    (ptr as usize
