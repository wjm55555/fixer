        ...
        match code {
            b'f' => {
                if pos_check_keyword(cur, function_keyword.as_bytes(), function_keyword_len) {
                    token_read_keyword(tok, cur, TOKEN_TYPE_FUNCTION, function_keyword.as_bytes(), function_keyword_len);
                } else {
            ...
            b'l' => {
                if pos_check_keyword(cur, let_keyword.as_bytes(), let_keyword_len) {
                    token_read_keyword(tok, cur, TOKEN_TYPE_LET, let_keyword.as_bytes(), let_keyword_len);
                } else if pos_check_keyword(cur, len_keyword.as_bytes(), len_keyword_len) {
                    token_read_keyword(tok, cur, TOKEN_TYPE_LEN, len_keyword.as_bytes(), len_keyword_len);
                } else {
                    token_read_ident(tok, cur);
                }
            }
            b'i' => {
                if pos_check_keyword(cur, if_keyword.as_bytes(), if_keyword_len) {
                    token_read_keyword(tok, cur, TOKEN_TYPE_IF, if_keyword.as_bytes(), if_keyword_len);
                } else {
                    token_read_ident(tok, cur);
                }
            }
            b'e' => {
                if pos_check_keyword(cur, else_keyword.as_bytes(), else_keyword_len) {
                    token_read_keyword(tok, cur, TOKEN_TYPE_ELSE, else_keyword.as_bytes(), else_keyword_len);
                } else {
                    token_read_ident(tok, cur);
                }
            }
            b'w' => {
                if pos_check_keyword(cur, while_keyword.as_bytes(), while_keyword_len) {
                    token_read_keyword(tok, cur, TOKEN_TYPE_WHILE, while_keyword.as_bytes(), while_keyword_len);
                } else {
                    token_read_ident(tok, cur);
                }
            }
            b'b' => {
                if pos_check_keyword(cur, break_keyword.as_bytes(), break_keyword_len) {
                    token_read_keyword(tok, cur, TOKEN_TYPE_BREAK, break_keyword.as_bytes(), break_keyword_len);
                } else {
                    token_read_ident(tok, cur);
                }
            }
            b'c' => {
                if pos_check_keyword(cur, continue_keyword.as_bytes(), continue_keyword_len) {
                    token_read_keyword(tok, cur, TOKEN_TYPE_CONTINUE, continue_keyword.as_bytes(), continue_keyword_len);
                } else {
                    token_read_ident(tok, cur);
                }
            }
            b'a' => {
                if pos_check_keyword(cur, append_keyword.as_bytes(), append_keyword_len) {
                    token_read_keyword(tok, cur, TOKEN_TYPE_APPEND, append_keyword.as_bytes(), append_keyword_len);
                } else {
                    token_read_ident(tok, cur);
                }
            }
            b'd' => {
                if pos_check_keyword(cur, delete_keyword.as_bytes(), delete_keyword_len) {
                    token_read_keyword(tok, cur, TOKEN_TYPE_DELETE, delete_keyword.as_bytes(), delete_keyword_len);
                } else {
                    token_read_ident(tok, cur);
                }
            }
            b'h' => {
                if pos_check_keyword(cur, has_property_keyword.as_bytes(), has_property_keyword_len) {
                    token_read_keyword(tok, cur, TOKEN_TYPE_HAS_PROPERTY, has_property_keyword.as_bytes(), has_property_keyword_len);
                } else {
                    token_read_ident(tok, cur);
                }
            }
            b'r' => {
                if pos_check_keyword(cur, return_keyword.as_bytes(), return_keyword_len) {
                    token_read_keyword(tok, cur, TOKEN_TYPE_RETURN, return_keyword.as_bytes(), return_keyword_len);
                } else {
                    token_read_ident(tok, cur);
                }
            }
        ...
        if let Some(ref t) = tok {
            if t.token_type == TOKEN_TYPE_UNKNOWN {
                let program_name_str = String::from_utf8_lossy(
                    &lexer.program_name[..lexer.program_name.iter().position(|&b| b == 0).unwrap_or(256)]
                );
                eprintln!(
    ...
    loop {
        ...
        if let Some(t) = tok {
            if t.token_type != TOKEN_TYPE_EOF {
                ...
                token_to_xml_string(&t, &mut tmp_str, 4096);
...
pub fn lexer_free(_lexer: Box<LEXER>) {
}