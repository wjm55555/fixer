...
fn dump_function_decl_ast_to_file_inner<'a>(
    ...
    spaces_num += 2;
    let _ = write!(f, "{:width$}", "", spaces_num);
    let _ = writeln!(f, "<function_name line=\"{}\" pos=\"{}\">", ast.line, ast.pos);
    spaces_num += 2;
...
fn dump_body_ast_to_file_inner<'a>(
    f: &'a mut dyn Write,
    ast: &'a BODY_AST,
    mut spaces_num: usize,
) {
...
pub fn create_append_stmt_ast(
    ...
    Box::new(APPEND_STMT_AST { arr, ident, line, pos })
}
...
pub fn create_delete_stmt_ast(
    ...
    Box::new(DELETE_STMT_AST { var, ident, line, pos })
}
...
fn dump_function_call_ast_to_file_inner<'a>(
    ...
    let _ = write!(f, "{:width$}", "", spaces_num);
    let _ = writeln!(f, "<function_call line=\"{}\" pos=\"{}\">", ast.line, ast.pos);
    spaces_num += 2;
    let _ = write!(f, "{:width$}", "", spaces_num);
    let _ = writeln!(f, "<function_name line=\"{}\" pos=\"{}\">", ast.line, ast.pos);
    spaces_num += 2;
